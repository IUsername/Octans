...
namespace Octans.Test
{
    public class ShadingTests
    {
        ...
        public void EyeBetweenLightAndSurface()
        {
            ...
            var eyeV = Vector.Create(0, 0, -1);
            var normalV = Vector.Create(0, 0, -1);
            var light = new PointLight(Point.Create(0, 0, -10), new Color(1f, 1f, 1f));
        ...
        public void EyeOffset45Deg()
        {
            var m = new Material();
            var position = Point.Create(0, 0, 0);
            var eyeV = Vector.Create(0, MathF.Sqrt(2) / 2, -MathF.Sqrt(2) / 2);
            var normalV = Vector.Create(0, 0, -1);
            var light = new PointLight(Point.Create(0, 0, -10), new Color(1f, 1f, 1f));
            var result = Shading.Lighting(m, light, position, eyeV, normalV);
        ...
        public void LightOffset45Deg()
        {
            var m = new Material();
            var position = Point.Create(0, 0, 0);
            var eyeV = Vector.Create(0, 0, -1f);
            var normalV = Vector.Create(0, 0, -1);
            var light = new PointLight(Point.Create(0, 10, -10), new Color(1f, 1f, 1f));
            var result = Shading.Lighting(m, light, position, eyeV, normalV);
