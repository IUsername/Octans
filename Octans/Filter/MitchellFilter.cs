...
namespace Octans.Filter
{
    ...
    public sealed class MitchellFilter : FilterBase
    {
        ...
        public override float Evaluate(in Point2D p)
        {
            return Eval1D(p.X * InverseRadius.X) * Eval1D(p.Y * InverseRadius.Y);
        }
        ...
        private float Eval1D(float x)
        {
            ...
            return ((12f - 9f * _b - 6f * _c) * x * x * x + (-18f + 12f * _b + 6f * _c) * x * x + (6f - 2f * _b)) *
                   (1f / 6f);
        }
    }
}