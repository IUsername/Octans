...
namespace Octans
{
    ...
    public static class Perlin
    {
        ...
        public static float Fbm(Point point, int octave)
        {
            var f = 0.0f;
            var w = 0.5f;
            for (var i = 0; i < octave; i++)
            {
                f += w * Noise(point.X, point.Y, point.Z);
                point *= 2.0f;
                w *= 0.5f;
            }
        ...
        public static float Fbm(float x, float y, float z, int octave) => Fbm(new Point(x, y, z), octave);
